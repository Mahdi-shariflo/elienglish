...
export function ThemeSwitcher() {
  ...
  const currentTheme: ThemeType = session?.theme || 'system';
  ...
  const applyTheme = async (themeName: ThemeType) => {
    if (themeName === 'dark') {
  ...
  const changeTheme = useCallback(async () => {
    if (isChanging) return; // جلوگیری از کلیک تند
    ...
    try {
      ...
      // بعد از 800ms دوباره قابل کلیک میشه
      setTimeout(() => {
        setIsChanging(false);
      }, 800);
    }
  ...
  // آیکون و متن بر اساس theme فعلی
    ...
    switch (theme) {
      ...
      case 'system':
        return (
          ...
          >
            <path
              d="M4 6C4 4.89543 4.89543 4 6 4H18C19.1046 4 20 4.89543 20 6V14C20 15.1046 19.1046 16 18 16H6C4.89543 16 4 15.1046 4 14V6ZM6 6V14H18V6H6Z"
              fill="currentColor"
            />
            <path
  ...
  return (
    <div className="flex items-center gap-2">
      <Button
        onPress={changeTheme}
        disabled={isChanging}
        className={`!h-[40px] !min-w-[40px] overflow-visible rounded-[12px] border border-[#E5EAEF] bg-transparent px-3 transition-all duration-300 lg:!h-[48px] lg:px-4 ${isChanging ? 'cursor-not-allowed opacity-50' : 'hover:bg-gray-50'} `}
      >
        {isChanging ? (
          <div className="flex items-center gap-1">
            <div className="h-1 w-1 animate-bounce rounded-full bg-[#6E3DFF]"></div>
            <div className="h-1 w-1 animate-bounce rounded-full bg-[#6E3DFF] delay-100"></div>
            <div className="h-1 w-1 animate-bounce rounded-full bg-[#6E3DFF] delay-200"></div>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <span className={`text-[#6E3DFF] ${isChanging ? 'animate-pulse' : ''}`}>
