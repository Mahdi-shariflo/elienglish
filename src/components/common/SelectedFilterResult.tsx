...
const SelectedFilterResult = () => {
  const searchParams = useSearchParams();
  const router = useRouter();

  const searchItemType = searchParams.get('searchItemType');

  if (!searchItemType || !filterLabels[searchItemType]) return null;

  const removeFilter = () => {
    const newParams = new URLSearchParams(searchParams.toString());
    newParams.delete('searchItemType');
...
export default SelectedFilterResult;
