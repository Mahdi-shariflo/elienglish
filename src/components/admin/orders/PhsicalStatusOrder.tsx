...
import { useUpdatePhysicalOrderById } from '@/hooks/admin/orders/physical/useUpdatePhysicalOrderById';
...
const PhsicalStatusOrder = ({ order }: { order: Order }) => {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const pathname = usePathname();
  const { mutate, isPending: isLoading, isSuccess } = useUpdatePhysicalOrderById();
  ...
  useEffect(() => {
    if (order && !formik.values.status) {
      router.push(`${pathname}/?status=${order.productPhysicalItems?.status}`);

      formik.setFieldValue('status', order.productPhysicalItems?.status);
    }
  }, [order]);
  useEffect(() => {
    if (isSuccess) {
...
export default PhsicalStatusOrder;
