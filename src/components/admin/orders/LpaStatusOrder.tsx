...
import { useUpdateLpaById } from '@/hooks/admin/orders/lpa/useUpdateLpaById';
...
const LpaStatusOrder = ({ order }: { order: Order }) => {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const pathname = usePathname();
  const { mutate, isPending: isLoading, isSuccess } = useUpdateLpaById();
  ...
  useEffect(() => {
    if (order && !formik.values.status) {
      router.push(`${pathname}/?status=${order.levelItems?.status}`);

      formik.setFieldValue('status', order.levelItems?.status);
    }
  }, [order]);
  useEffect(() => {
    if (isSuccess) {
