...
const DiscountOrder = ({ order }: { order: Order }) => {
  if (!order?.orderDiscountPrice) return null;
  return (
    <div className="mt-3 rounded-xl border border-[#E4E7E9] p-4">
      ...
      <div className="mt-3 space-y-3">
        <p className="font-regular text-[14px] text-[#7D8793]">
          کد تخفیف: <span className="text-[#0C0C0C]">{order?.orderDiscountCode}</span>
        </p>
        <p className="font-regular text-[14px] text-[#7D8793]">
          نوع کد تخفیف:{' '}
          <span className="text-[#0C0C0C]">
            {order?.orderDiscountType === 'fixed' ? 'ثابت' : 'درصد'}
          </span>
        </p>
        <p className="font-regular text-[14px] text-[#7D8793]">
          مبلغ کد تخفیف:{' '}
          <span className="text-[#0C0C0C]">
            {Number(order?.orderDiscountPrice).toLocaleString()} تومان
          </span>
        </p>
      </div>
    </div>
