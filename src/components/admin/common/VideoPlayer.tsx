...
const VideoPlayer = ({ url, poster }: { url: string; poster?: string }) => {
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      localHls.on(Hls.Events.MANIFEST_PARSED, () => {
        const levelList = localHls!.levels.map((level, index) => ({
          height: level.height,
          index,
        }));
        setLevels(levelList);
        setCurrentLevel(localHls!.currentLevel);
        video.play();
      });
      setHls(localHls);
    ...
    const updateProgress = () => setPlayed(video.currentTime);
  ...
  return (
    <div className="video-player relative h-full w-full" onClick={togglePlay}>
      {!isPlaying && poster ? (
        <img src={poster} alt="poster" className="absolute z-10 h-full w-full object-cover" />
      ) : null}
