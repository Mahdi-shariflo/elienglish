...
const FormChapters = ({ open, setOpen, formik, idx }: Props) => {
  ...
  // state برای track کردن حالت ویرایش
  const [editingIndex, setEditingIndex] = useState<number | null>(null);

  // state برای کنترل accordion
  ...
  const addEpisode = () => {
    ...
    if (editingIndex !== null) {
      // حالت ویرایش - جایگزین کردن اپیزود موجود
      const updatedEpisodes = [...currentEpisodes];
      updatedEpisodes[editingIndex] = { ...newEpisode };
  ...
  const removeEpisode = (epIndexToRemove: number) => {
    ...
    // اگر در حال ویرایش همین آیتم بودیم، ویرایش رو لغو کن
  ...
  const startEditing = (epIndex: number) => {
    const episode = formik.values.chapters[idx]?.episodes[epIndex];
    if (episode) {
      setNewEpisode({
        title: episode.title || '',
        order: episode.order || '',
      ...
      // باز کردن accordion دوم (اپیزودها) وقتی ویرایش شروع میشه
      setExpandedKeys(new Set(['1', '2']));
    }
  };
    ...
    >
      <div>
        <Accordion
          ...
          onSelectionChange={(keys) => setExpandedKeys(new Set(Array.from(keys) as string[]))}
        >
          <AccordionItem
            key="1"
          ...
          <AccordionItem
            ...
            classNames={{ title: 'font-medium', base: 'border-b' }}
            title={editingIndex !== null ? 'ویرایش اپیزود' : 'اپیزود های دوره'}
          >
            <div className="mt-5 grid grid-cols-2 gap-4 rounded-lg border p-4">
              ...
              <Select
                ...
                onChange={(value) => setNewEpisode({ ...newEpisode, type: value?.value })}
                options={[
              ...
              <div className="flex gap-2">
                <Button className="flex-1 bg-main text-white" onClick={addEpisode}>
                  {editingIndex !== null ? 'بروزرسانی اپیزود' : 'ذخیره اپیزود'}
                </Button>
                {editingIndex !== null && (
                  <Button className="flex-1 bg-gray-500 text-white" onClick={cancelEditing}>
                    لغو ویرایش
                  </Button>
                )}
              </div>
          ...
          {(formik.values.chapters[idx]?.episodes || []).map((ep: any, epIndex: number) => (
            <div
              className={`flex items-center justify-between ${
                editingIndex === epIndex ? 'rounded-lg border border-blue-200 bg-blue-50 p-2' : ''
              }`}
              key={epIndex}
            >
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`font-extrabold text-[16px] ${
                    editingIndex === epIndex ? 'text-blue-600' : 'text-main'
                  }`}
                >
                  {ep.title}
                  {editingIndex === epIndex && (
                    <span className="mr-2 text-sm text-blue-500">(در حال ویرایش)</span>
                  )}
                </span>
              </div>
              <div className="flex h-[50px] items-center gap-4 rounded-lg px-4">
                ...
                {/* دکمه ویرایش */}
                <Button
                  className="h-fit w-fit bg-blue-500 text-white"
                  onClick={() => startEditing(epIndex)}
                ...
                <Button className="block h-5 w-5" onClick={() => removeEpisode(epIndex)}>
                  <Delete_icon className="text-gray-400" />
                </Button>
              </div>
            </div>
          ))}
