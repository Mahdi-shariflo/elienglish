...
export default function SelectBlog({ values, onChange, title, className }: Props) {
  const [search, setSearch] = useState('');
  const [selectedProducts, setSelectedProducts] = useState<Blog[]>(values);
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState<Product[]>([]);
  const [page, setPage] = useState(1);
  const { isLoading, isSuccess, isFetching, data, isPending } = useGetBlogsAdmin({
    page: page.toString(),
    search,
  });
  ...
  useEffect(() => {
    if (isSuccess) {
      const products = data?.data?.data?.blogs;
