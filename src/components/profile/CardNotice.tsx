...
const CardNotice = () => {
  ...
  return (
    <div className="rounded-lg bg-white p-3 shadow-favorite">
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="font-regular text-[14px] text-[#232429]">
            اطلاع رسانی ارسال بسته های پستی{' '}
          </p>
          <p
            className={`pt-1 font-regular text-[12px] text-[#7D8793] ${open ? '' : 'line-clamp-2'}`}
          >
            سلام دوست زیبای من، خیلی ممنون که رز سفید رو برای خریدتون انتخاب کردی. دوستان خوبم اداره
            پست به مناسبت روز یکم فروردین، تعطیل هستش. برای همین لطفاً در نظر داشته باش که سفارشاتی
            که توی این روز ثبت می‌شن، پس از تعطیلات پستی، در روز بعد به شما ارسال میشه . اما سفارشات
            قبل و بعد از این تاریخ به روال روزانه پردازش و ارسال خواهند شد و با تأخیری مواجه نخواهند
            شد. با توجه به اینکه اداره پست پس از تعطیلات، به صورت روزانه در تمامی روزهای هفته کار
            خود را انجام می‌ده، می‌توانید با خیالی آسوده سفارش خودتون رو ثبت کنید و به زودی دریافتش
            کنید. مرسی که با ما همراهید و از خدمات ما استفاده می‌کنید. در صورت بروز هرگونه سوال یا
            مشکل، می‌تونید با پشتیبانی ما تماس بگیرید تا بهترین خدمات رو به شما ارائه بدیم.
          </p>
        </div>
      </div>
      <span className="flex justify-end">
        <Button onClick={() => setOpen(!open)} className="w-fit text-[12px] text-main">
