...
const SelectedFilterCourse = () => {
  const searchParams = useSearchParams();
  const router = useRouter();

  const statusCourse = searchParams.get('statusCourse');

  if (!statusCourse || !filterLabels[statusCourse]) return null;

  const removeFilter = () => {
    const newParams = new URLSearchParams(searchParams.toString());
    newParams.delete('statusCourse');
...
export default SelectedFilterCourse;
